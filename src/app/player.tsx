  ...
  return (
    <View className="flex-1 bg-white px-6 pt-4">
      ...
      <View className="flex-row items-center mt-8 gap-3">
        <Image className="w-12 h-12 rounded-lg" source={{ uri: episode.feedImage }} />

        <View className="flex-1">
          <Text className="text-xs text-gray-400">{episode.datePublishedPretty}</Text>
      ...
      <View className="flex-row items-center justify-between mt-4 px-4">
        <Pressable onPress={cyclePlaybackRate}>
          <Text className="text-black text-base font-medium">{player.playbackRate}x</Text>
        </Pressable>
        ...
        <Pressable
          onPress={() => {
            if (playerStatus.playing) {
              player.pause();
              console.log("🚀 player.tsx -> #10 ~ player.pause()");
            } else {
              player.play();
              console.log("🚀 player.tsx -> #11 ~ player.play()");
            }
          }}
